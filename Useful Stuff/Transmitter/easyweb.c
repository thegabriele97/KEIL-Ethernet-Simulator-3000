  ...
  *((unsigned char *)RemoteIP + 3) = 134;
  ...
  while (1) {
		...
		if (!(SocketStatus & SOCK_CONNECTED)) {
			...
			while(!(SocketStatus & SOCK_CONNECTED)) {
				DoNetworkStuff();
			}
		}
...
void DoNetworkWork(void) {
	int adc_val;
	uint8_t i, thermometer = 0;
	
  if (SocketStatus & SOCK_CONNECTED) {             // check if socket is open
		if (SocketStatus & SOCK_TX_BUF_RELEASED) {		// check if we can send to remote TCP
			
			adc_val = GetAD7Val();
			for (i = 1; i <= (adc_val >> 9); i++) {
				thermometer |= 1 << (i - 1);
			}
			
			LPC_GPIO2->FIOPIN = (LPC_GPIO2->FIOPIN & 0x80) | (thermometer & 0x7f);
			memcpy(TCP_TX_BUF, &thermometer, sizeof thermometer);
			TCPTxDataCount = sizeof thermometer;
			TCPTransmitTxBuffer();
		}
	}
}
